...
from lib.train.utils import get_hparams_from_file, load_checkpoint
...
def __speecht5__(text, speaker_embedding=None, device="cpu"):
    ...
    tts_vocoder = SpeechT5HifiGan.from_pretrained(
        speecht5_vocoder_checkpoint,
        cache_dir=os.path.join(TTS_MODELS_DIR, speecht5_vocoder_checkpoint),
        device_map=device,
    )
    tts_processor = SpeechT5Processor.from_pretrained(
        speecht5_checkpoint,
        cache_dir=os.path.join(TTS_MODELS_DIR, speecht5_checkpoint),
        device_map=device,
    )
    ...
    if speaker_embedding is None:
        raise ValueError("speaker_embedding is required for SpeechT5 inference")
    speaker_embedding = (
        speaker_embedding
        if hasattr(speaker_embedding, "to")
        else torch.tensor(speaker_embedding)
    )

    dtype = torch.float32 if "cpu" in str(device) else torch.float16
    speech = tts_model.generate_speech(
        input_ids.to(device),
        speaker_embedding.to(device).to(dtype),
        vocoder=tts_vocoder,
    )
...
def __bark__(text, device="cpu"):
    ...
    inputs = bark_processor(
        text=[text], return_tensors="pt", voice_preset=bark_voice_presets
    )
    tensor_dict = {
...
def __tacotron2__(text, device="cpu"):
    ...
    hifi_gan = HIFIGAN.from_hparams(
        source=hifigan_checkpoint,
        savedir=os.path.join(TTS_MODELS_DIR, hifigan_checkpoint),
        run_opts={"device": device},
    )
    tacotron2 = Tacotron2.from_hparams(
        source=tacotron2_checkpoint,
        savedir=os.path.join(TTS_MODELS_DIR, tacotron2_checkpoint),
        run_opts={"device": device},
    )
    # Running the TTS
...
def __edge__(text, speaker="en-US-JennyNeural"):
    ...
    tempfile = os.path.join(
        temp_dir, f"{hashlib.md5(text.encode('utf-8')).hexdigest()}.wav"
    )
    ...
    thread = Thread(
        target=asyncio.run, args=(fetch_audio(),), name="edge-tts", daemon=True
    )
    thread.start()
    thread.join()
...
def __silero__(text, speaker="lj_16khz"):
    silero_tts = importlib.import_module("silero").silero_tts

    model, symbols, sample_rate, _, apply_tts = silero_tts(
        repo_or_dir="snakers4/silero-models", language="en", speaker=speaker
    )
...
def __vits__(text, speaker=os.path.join(BASE_MODELS_DIR, "VITS", "pretrained_ljs.pth")):
    ...
    hps = get_hparams_from_file(
        os.path.join(BASE_MODELS_DIR, "VITS", "configs", "ljs_base.json")
    )
    ...
    _ = load_checkpoint(speaker, net_g, None)
...
def generate_speech(
    text, speaker=None, method="speecht5", device="cpu", dialog_only=False
):
    ...
    if method == "speecht5":
        if isinstance(speaker, str):
            embedding_path = os.path.join(
                TTS_MODELS_DIR, "embeddings", f"{speaker}.npy"
            )
            if os.path.isfile(embedding_path):
                ...
                raise ValueError(
                    f"Must provider a speaker_embedding for {method} inference!"
                )
        else:
...
def load_stt_models(method="vosk", recognizer=None):
    if method == "vosk":
        ...
        Model = importlib.import_module("vosk").Model
        ...
        return {"recognizer": recognizer, "model": model}
    elif method == "speecht5":
        from transformers import SpeechT5Processor, SpeechT5ForSpeechToText

        processor = SpeechT5Processor.from_pretrained(
            stt_checkpoint, cache_dir=os.path.join(STT_MODELS_DIR, stt_checkpoint)
        )
        generator = SpeechT5ForSpeechToText.from_pretrained(
            stt_checkpoint, cache_dir=os.path.join(STT_MODELS_DIR, stt_checkpoint)
        )

        return {"processor": processor, "generator": generator}
...
def transcribe_speech(input_audio, stt_models=None, stt_method="vosk", denoise=False):
    ...
    if stt_method == "vosk":
        ...
        input_data = recognizer.recognize_vosk(input_audio)
        ...
        for slice in slicer.slice(audio):
            ...
            audio_len = (
                int(len(slice) * 6.25 // sr) + 1
            )  # average 2.5 words/s spoken at 2.5 token/word
