...
class VC(FeatureExtractor):
    ...
    def pipeline(
        ...
        if audio_pad.shape[0] > self.t_max:
            ...
            for t in range(self.t_center, audio.shape[0], self.t_center):
                ...
                opt_ts.append(
                    t
                    - self.t_query
                    + np.where(abs_audio_sum == min_abs_audio_sum)[0][0]
                )
        ...
        if if_f0:
            ...
            pitch = pitch[:p_len].astype(
                np.int64 if self.device != "mps" else np.float32
            )
        ...
        for i, t in enumerate(opt_ts):
            ...
            pitch_slice = (
                pitch[:, start // self.window : end // self.window] if if_f0 else None
            )
            pitchf_slice = (
                pitchf[:, start // self.window : end // self.window] if if_f0 else None
            )
            audio_opt.append(
                self.vc(
                    model,
    ...
    try:  # preload file_index
        if len(index_files) == 0:
            ...
            if os.path.exists(file_index):
                ...
                sys.stdout.write(
                    f"Attempting to load {file_index}.... (despite it not existing)\n"
                )
                sys.stdout.flush()
    ...
    **kwargs,  # prevents function from breaking
    ...
    try:
        audio = (
            ...
            else load_input_audio(input_audio_path, 16000)
        )
        ...
        audio_opt = vc.pipeline(
            ...
            f0_method
            if len(f0_method) > 1
            else f0_method[0],  # more than 1 f0_method in list means hybrid
            merge_type,
            file_index,
            index_rate,
            if_f0,
