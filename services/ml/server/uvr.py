...
def list_uvr_denoise_models():
    fnames = get_filenames(
        root=BASE_MODELS_DIR, name_filters=["echo", "reverb", "noise", "kara"]
    )
    return [os.path.relpath(name, BASE_MODELS_DIR) for name in fnames]
...
def split_vocals(
    ...
    audio_data: Union[str, Any],
    **kwargs,
):
    try:
        input_audio = (
            bytes2audio(audio_data) if isinstance(audio_data, str) else audio_data
        )
        if input_audio:
            cache_key = audio_data if isinstance(audio_data, str) else repr(kwargs)
            tempfile = os.path.join(
                CACHE_DIR, f"{hashlib.md5(cache_key.encode('utf-8')).hexdigest()}.wav"
            )
            if not os.path.isfile(tempfile):
                audio_array = input_audio[0]
                save_input_audio(
                    tempfile,
                    input_audio,
                    to_int16=hasattr(audio_array, "dtype")
                    and "int" in audio_array.dtype.name,
                )
