...
class Separator:
    def __init__(
        self, model_path, use_cache=False, device="cpu", cache_dir=None, **kwargs
    ):
        ...
        if "MDX" in model_path:
            self.model = MDXNet(
                model_path=model_path, denoise=denoise, device=device, **kwargs
            )
        elif "UVR" in model_path:
            self.model = (
                UVR5New(
                    model_path=model_path, device=device, dereverb=dereverb, **kwargs
                )
                if denoise
                else UVR5Base(model_path=model_path, device=device, **kwargs)
            )
    ...
    def run_inference(self, audio_path, format="mp3"):
        song_name = (
            get_filename(os.path.basename(self.model_path).split(".")[0], **self.args)
            + f".{format}"
        )
...
def __run_inference_worker(arg):
    (model_path, audio_path, agg, device, use_cache, cache_dir, num_threads, format) = (
        arg
    )
    if "karafan" in model_path:
        vocals, instrumental, input_audio = karafan.inference.Process(
            audio_path, cache_dir=cache_dir, use_cache=use_cache, format=format
        )
    else:
        model = Separator(
            agg=agg,
...
def split_audio(
    ...
    **kwargs,
    ...
    if len(preprocess_models):
        output_name = (
            get_filename(
                *[os.path.basename(name).split(".")[0] for name in preprocess_models],
                agg=agg,
            )
            + f".{format}"
        )
        ...
        if os.path.isfile(preprocessed_file):
            input_audio = load_input_audio(preprocessed_file, mono=True)
        else:  # preprocess audio
            for i, preprocess_model in enumerate(preprocess_models):
                output_name = (
                    get_filename(
                        i, os.path.basename(preprocess_model).split(".")[0], agg=agg
                    )
                    + f".{format}"
                )
                intermediary_file = os.path.join(
                    cache_dir, "preprocessing", output_name
                )
                if os.path.isfile(intermediary_file):
                    # if i==len(preprocess_model)-1: #last model
                    instrumental = input_audio = load_input_audio(
                        intermediary_file, mono=True
                    )
                else:
                    args = (
                        preprocess_model,
                        ...
                        device,
                        False,
                        CACHED_SONGS_DIR if i == 0 else None,
                        num_threads,
                        format,
                    )
    ...
    if len(postprocess_models):
        vocals_name = (
            get_filename(
                "vocals",
                *[os.path.basename(name).split(".")[0] for name in uvr_models],
                agg=agg,
            )
            + f".{format}"
        )
        ...
        if not os.path.isfile(vocals_file):
            save_input_audio(vocals_file, (wav_vocals, vocals[-1]), to_int16=True)
        print("postprocessing...")
        for i, postprocess_model in enumerate(postprocess_models):
            output_name = (
                get_filename(
                    i, os.path.basename(postprocess_model).split(".")[0], agg=agg
                )
                + f".{format}"
            )
...
def main():  # uvr5_models,audio_path,device="cuda",agg=10,use_cache=False
    ...
    parser = argparse.ArgumentParser(
        description="processes audio to split vocal stems and reduce reverb/echo"
    )

    parser.add_argument(
        "uvr5_models", type=str, nargs="+", help="Path to models to use for processing"
    )
    parser.add_argument(
        "-i",
        "--audio_path",
    ...
    parser.add_argument(
        ...
        "--preprocess_model",
        type=str,
        help="preprocessing model to improve audio",
        default=None,
    )
    parser.add_argument(
        "-a",
        ...
        type=int,
        default=10,
        help="aggressiveness score for processing (0-20)",
    )
    parser.add_argument(
        "-d",
        "--device",
        ...
        default="cpu",
        choices=["cpu", "cuda"],
        help="perform calculations on [cpu] or [cuda]",
    )
    parser.add_argument(
        "-m",
        "--merge_type",
        ...
        default="median",
        choices=["mean", "median"],
        help="how to combine processed audio",
    )
    parser.add_argument(
        "-c",
        "--use_cache",
        ...
        action="store_true",
        default=False,
        help="caches the results so next run is faster",
    )
