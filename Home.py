...
def download_audio_to_buffer(url, subtype="mp4"):
    ...
    if audio is None:
        raise ValueError(f"No audio stream available for subtype={subtype!r}")
...
def render_model_checkboxes(generator):
    ...
    for model_path, link in generator:
        ...
        if not is_downloaded:
            not_downloaded.append((model_path, link))
...
def rvc_index_path_mapper(params):
    ...
    if "index" not in data_path.split(".")[-1]:
        ...
        return (
            os.path.join(BASE_MODELS_DIR, "RVC", ".index", os.path.basename(data_path)),
            data,
        )  # index file
...
if __name__ == "__main__":
    ...
    with model_tab:
        ...
        with st.expander("Base Models"):
            ...
            if st.button(
                "Download All",
                key="download-all-base-models",
                disabled=len(to_download) == 0,
            ):
                with ProgressBarContext(
                    to_download, file_downloader, "Downloading models"
                ) as pb:
                    pb.run()
        ...
        with st.expander("Pretrained Models"):
            ...
            if st.button(
                "Download All",
                key="download-all-pretrained-models",
                disabled=len(to_download) == 0,
            ):
                with ProgressBarContext(
                    to_download, file_downloader, "Downloading models"
        ...
        with st.container():
            if platform.system() == "Windows":
                ...
                st.markdown(
                    "run `apt update && apt install -y -qq ffmpeg espeak` in your terminal"
                )

        st.subheader("Required Models for inference")
        with st.expander("RVC Models"):
            file_uploader_form(
                os.path.join(BASE_MODELS_DIR, "RVC"),
                "Upload your RVC model",
                types=["pth", "index", "zip"],
        ...
        with st.expander("Vocal Separation Models"):
            generator = download_link_generator(
                RVC_DOWNLOAD_LINK, VR_MODELS + MDX_MODELS + KARAFAN_MODELS
            )
            to_download = render_model_checkboxes(generator)
            if st.button(
                "Download All",
                key="download-all-vr-models",
                disabled=len(to_download) == 0,
            ):
                with ProgressBarContext(
                    to_download, file_downloader, "Downloading models"
    ...
    with audio_tab, SessionStateContext("youtube_downloader") as state:
        ...
        state.url = st.text_input(
            "Insert Youtube URL:", value=state.url if state.url else ""
        )
