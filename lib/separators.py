...
class UVR5Base:
    ...
    def process_vocals(
        self, v_spec_m, input_high_end, input_high_end_h, return_dict={}
    ):
        ...
        return_dict["vocals"] = remix_audio(
            (wav_vocals, return_dict["sr"]), to_int16=True, axis=0
        )
        return return_dict["vocals"]

    def process_instrumental(
        self, y_spec_m, input_high_end, input_high_end_h, return_dict={}
    ):
        if self.data["high_end_process"].startswith("mirroring"):
            input_high_end_ = spec_utils.mirroring(
        ...
        return_dict["instrumentals"] = remix_audio(
            (wav_instrument, return_dict["sr"]), to_int16=True, axis=0
        )
        return return_dict["instrumentals"]

    def process_audio(self, y_spec_m, v_spec_m, input_high_end, input_high_end_h):
        return_dict = {"sr": self.mp.param["sr"]}

        with ThreadPool(2) as pool:
            pool.apply(
                self.process_vocals,
                args=(v_spec_m, input_high_end, input_high_end_h, return_dict),
            )
            pool.apply(
                self.process_instrumental,
                args=(y_spec_m, input_high_end, input_high_end_h, return_dict),
            )
    ...
    def run_inference(self, music_file):
        ...
        for d in range(bands_n, 0, -1):
            ...
            if d == bands_n:  # high-end band
                input_audio = librosa.core.load(
                    music_file, sr=bp["sr"], res_type=bp["res_type"]
                )
        ...
        return_dict = self.process_audio(
            y_spec_m, v_spec_m, input_high_end, input_high_end_h
        )
