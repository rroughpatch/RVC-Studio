...
def align_audio(
    ...
    for i in range(check_range):
        index = int(
            random.uniform(44100 * 2, min(wav1.shape[0], wav2.shape[0]) - 44100 * 2)
        )
    ...
    def save_aligned_audio(wav2_aligned):
        command_Text("Aligned File 2 with File 1.\n")
        command_Text("Saving files... ")
        sf.write(
            file2_aligned,
            normalize(wav2_aligned, is_normalization),
            sr2,
            subtype=wav_type_set,
        )
    ...
    if est_diff > 0:
        ...
        command_Text("Audio files already aligned.\n")
    ...
    sf.write(
        file_subtracted, normalize(wav_sub, is_normalization), sr1, subtype=wav_type_set
    )
