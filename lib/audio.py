    ...
    **kwargs,
    ...
    if resample or input_audio[1] != target_sr:
        audio = librosa.core.resample(
            np.array(input_audio[0], dtype="float32"),
            orig_sr=input_audio[1],
            target_sr=target_sr,
            **kwargs,
        )

    if audio.ndim > 1:
        audio = np.nanmedian(audio, axis=axis)
    if norm:
        audio = librosa.util.normalize(audio, axis=axis)

    audio_max = np.abs(audio).max() / 0.99
    if audio_max > 1:
        audio = audio / audio_max

    if to_int16:
        audio = np.clip(
            audio * MAX_INT16, a_min=-MAX_INT16 + 1, a_max=MAX_INT16 - 1
        ).astype("int16")
    print(
...
def load_input_audio(fname, sr=None, **kwargs):
    ...
    print(
        f"loading sound {fname=} {audio.ndim=} {audio.max()=} {audio.min()=} {audio.dtype=} {sr=}"
    )
    return audio, sr
...
def save_input_audio(fname, input_audio, sr=None, to_int16=False, to_stereo=False):
    ...
    try:
        sf.write(
            fname,
            audio.astype("int16" if np.abs(audio).max() > 1 else "float32"),
            sr if sr else input_audio[1],
        )
...
def audio2bytes(audio: np.ndarray[Any, Any], sr: int):
    try:
...
def pad_audio(*audios, axis=0):
    ...
    if maxlen > 0:
        stack = librosa.util.stack(
            [
                librosa.util.pad_center(data=a, size=maxlen)
                for a in audios
                if a is not None
            ],
            axis=axis,
...
def merge_audio(audio1, audio2, sr=40000):
    print(
        f"merging audio audio1={audio1[0].shape, audio1[1]} audio2={audio2[0].shape, audio2[1]} sr={sr}"
    )
...
def autotune_f0(f0, threshold=0.0):
    # autotuned_f0 = []
    # for freq in f0:
