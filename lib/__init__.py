...
class ObjectNamespace(dict):
    ...
    def get(self, name: str, default_value=None):
        return self.__getitem__(name) if name in self.keys() else default_value

    def __getattr__(self, name: str):
        return self.__getitem__(name) if name in self.keys() else ObjectNamespace()
    ...
    def __delattr__(self, name: str):
        return self.__delitem__(name) if name in self.keys() else None

    def __delitem__(self, name: str):
        return super().__delitem__(name) if name in self.keys() else None
...
class PersistedDict:
    ...
    @contextmanager
    def open_shelf(self) -> Iterator[MutableMapping[str, object]]:
        ...
        try:
            ...
            close = getattr(shelf, "close", None)
            if callable(close):
                close()
