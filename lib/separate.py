...
from lib.uvr5_pack.demucs.apply import apply_model, demucs_segments
from lib.uvr5_pack.demucs.hdemucs import HDemucs
from lib.uvr5_pack.demucs.model_v2 import auto_load_demucs_model_v2
from lib.uvr5_pack.demucs.pretrained import get_model as _gm
from lib.uvr5_pack.demucs.utils import apply_model_v1
from lib.uvr5_pack.demucs.utils import apply_model_v2
from lib.uvr5_pack import spec_utils
from lib.uvr5_pack.vr_network import nets
from lib.uvr5_pack.vr_network import nets_new

# from lib_v5.vr_network.model_param_init import ModelParameters
from pathlib import Path
from lib.uvr5_pack.constants import *
from lib.uvr5_pack.error_handling import *
import audioread
import gzip
import librosa
import math
...
import traceback
import lib.uvr5_pack.mdxnet as MdxnetSet

if TYPE_CHECKING:
    from lib.uvr5_pack.UVR import ModelData
...
class SeperateAttributes:
    def __init__(
        ...
        self.is_secondary_model_activated = (
            model_data.is_secondary_model_activated
        ...
        self.is_primary_stem_only = (
            model_data.is_primary_stem_only
            if not self.is_secondary_model
            else model_data.is_primary_model_primary_stem_only
        )
        self.is_secondary_stem_only = (
            model_data.is_secondary_stem_only
            if not self.is_secondary_model
            else model_data.is_primary_model_secondary_stem_only
        )
        ...
        self.is_demucs_pre_proc_model_inst_mix = (
            model_data.is_demucs_pre_proc_model_inst_mix
        )  #
        ...
        if not model_data.process_method == DEMUCS_ARCH_TYPE:
            if process_data["is_ensemble_master"] and not self.is_4_stem_ensemble:
                if not model_data.ensemble_primary_stem == self.primary_stem:
                    self.is_primary_stem_only, self.is_secondary_stem_only = (
                        self.is_secondary_stem_only,
                        self.is_primary_stem_only,
                    )
            ...
            if self.is_secondary_model and not process_data["is_ensemble_master"]:
                if (
                    not self.primary_model_primary_stem == self.primary_stem
                    and not main_model_primary_stem_4_stem
                ):
                    self.is_primary_stem_only, self.is_secondary_stem_only = (
                        self.is_secondary_stem_only,
                        self.is_primary_stem_only,
            ...
            if main_model_primary_stem_4_stem:
                self.is_primary_stem_only = (
                    True
                    if main_model_primary_stem_4_stem == self.primary_stem
                    else False
                )
                self.is_secondary_stem_only = (
                    True
                    if not main_model_primary_stem_4_stem == self.primary_stem
                    else False
                )

            if self.is_pre_proc_model:
                self.is_primary_stem_only = (
                    True if self.primary_stem == INST_STEM else False
                )
                self.is_secondary_stem_only = (
                    True if self.secondary_stem == INST_STEM else False
                )

        if model_data.process_method == MDX_ARCH_TYPE:
            self.is_mdx_ckpt = model_data.is_mdx_ckpt
            self.primary_model_name, self.primary_sources = self.cached_source_callback(
                MDX_ARCH_TYPE, model_name=self.model_basename
            )
            self.is_denoise = model_data.is_denoise
            self.mdx_batch_size = model_data.mdx_batch_size
            self.compensate = model_data.compensate
            self.dim_f, self.dim_t = (
        ...
        if model_data.process_method == DEMUCS_ARCH_TYPE:
            ...
            self.is_split_mode = (
                model_data.is_split_mode
                if not self.demucs_version == DEMUCS_V4
                else True
            )
            self.overlap = model_data.overlap
            self.primary_model_name, self.primary_sources = self.cached_source_callback(
                DEMUCS_ARCH_TYPE, model_name=self.model_basename
            )

        if model_data.process_method == VR_ARCH_TYPE:
            self.primary_model_name, self.primary_sources = self.cached_source_callback(
                VR_ARCH_TYPE, model_name=self.model_basename
            )
    ...
    def start_inference_console_write(self):
        ...
        if self.is_secondary_model and not self.is_pre_proc_model:
            self.write_to_console(
                INFERENCE_STEP_2_SEC(self.process_method, self.model_basename)
            )

        if self.is_pre_proc_model:
            self.write_to_console(
                INFERENCE_STEP_2_PRE(self.process_method, self.model_basename)
            )
    ...
    def load_cached_sources(self, is_4_stem_demucs=False):
        ...
        if self.is_secondary_model and not self.is_pre_proc_model:
            self.write_to_console(
                INFERENCE_STEP_2_SEC_CACHED_MODOEL(
                    self.process_method, self.model_basename
                )
            )
        elif self.is_pre_proc_model:
            self.write_to_console(
        ...
        if not is_4_stem_demucs:
            primary_stem, secondary_stem = gather_sources(
                self.primary_stem, self.secondary_stem, self.primary_sources
            )
        ...
        if not model_occurrences <= 1:
            if self.process_method == MDX_ARCH_TYPE:
                self.cached_model_source_holder(
                    MDX_ARCH_TYPE, secondary_sources, self.model_basename
                )

            if self.process_method == VR_ARCH_TYPE:
                self.cached_model_source_holder(
                    VR_ARCH_TYPE, secondary_sources, self.model_basename
                )

            if self.process_method == DEMUCS_ARCH_TYPE:
                self.cached_model_source_holder(
                    DEMUCS_ARCH_TYPE, secondary_sources, self.model_basename
                )
            ...
            if self.is_secondary_model_activated:
                if isinstance(secondary_model_source, np.ndarray):
                    secondary_model_scale = (
                        model_scale if model_scale else self.secondary_model_scale
                    )
                    stem_source = spec_utils.average_dual_sources(
                        stem_source, secondary_model_source, secondary_model_scale
                    )

            sf.write(stem_path, stem_source, samplerate, subtype=self.wav_type_set)
            save_format(
                stem_path, self.save_format, self.mp3_bit_set
            ) if not self.is_ensemble_mode else None
...
class SeperateMDX(SeperateAttributes):
    def seperate(self):
        ...
        if self.primary_model_name == self.model_basename and self.primary_sources:
            ...
            if self.is_mdx_ckpt:
                ...
                self.model_run = (
                    separator.load_from_checkpoint(self.model_path)
                    .to(self.device)
                    .eval()
                )
            else:
            ...
            mix, raw_mix, samplerate = prepare_mix(
                self.audio_file, self.chunks, self.margin, mdx_net_cut=mdx_net_cut
            )
        ...
        if not self.is_secondary_stem_only:
            self.write_to_console(
                f"{SAVING_STEM[0]}{self.primary_stem}{SAVING_STEM[1]}"
            ) if not self.is_secondary_model else None
        ...
        if not self.is_primary_stem_only:
            self.write_to_console(
                f"{SAVING_STEM[0]}{self.secondary_stem}{SAVING_STEM[1]}"
            ) if not self.is_secondary_model else None
            secondary_stem_path = os.path.join(
                self.export_path, f"{self.audio_file_base}_({self.secondary_stem}).wav"
            )
            if not isinstance(self.secondary_source, np.ndarray):
                raw_mix = (
                    self.demix_base(raw_mix, is_match_mix=True)[0]
                    if mdx_net_cut
                    else raw_mix
                )
                self.secondary_source, raw_mix = spec_utils.normalize_two_stem(
                    source * self.compensate, raw_mix, self.is_normalization
                )
    ...
    def initialize_model_settings(self):
        ...
        self.window = torch.hann_window(window_length=self.n_fft, periodic=False).to(
            self.device
        )
            ...
            with torch.no_grad():
                for mix_wave in mix_waves:
                    self.running_inference_progress_bar(
                        len(mix) * len(mix_waves), is_match_mix=is_match_mix
                    )
                ...
                tar_waves_ = (
                    np.vstack(tar_waves_)[:, :, self.trim : -self.trim]
                    if is_ckpt
                    else tar_waves_
                )
...
class SeperateDemucs(SeperateAttributes):
    def seperate(self):
        ...
        if (
            ...
            and type(self.primary_sources) is dict
            ...
            if self.pre_proc_model:
                if self.primary_stem not in [VOCAL_STEM, INST_STEM]:
                    ...
                    mix_no_voc = process_secondary_model(
                        self.pre_proc_model, self.process_data, is_pre_proc_model=True
                    )
                    inst_mix, inst_raw_mix, inst_samplerate = prepare_mix(
                        mix_no_voc[INST_STEM], self.chunks_demucs, self.margin_demucs
                    )
            ...
            mix, raw_mix, samplerate = prepare_mix(
                self.audio_file, self.chunks_demucs, self.margin_demucs
            )
        ...
        if (
                ...
                if (
                    ...
                    if self.secondary_model_4_stem[stem_value]:
                        ...
                        stem_source_secondary = process_secondary_model(
                            self.secondary_model_4_stem[stem_value],
                            self.process_data,
                            main_model_primary_stem_4_stem=stem_name,
                            is_4_stem_demucs=True,
                        )
                        if isinstance(stem_source_secondary, np.ndarray):
                            stem_source_secondary = stem_source_secondary[
                                1
                                if self.secondary_model_4_stem[
                                    stem_value
                                ].demucs_stem_count
                                == 2
                                else stem_value
                            ]
                            stem_source_secondary = spec_utils.normalize(
                                stem_source_secondary, self.is_normalization
                            ).T
                        elif type(stem_source_secondary) is dict:
                            stem_source_secondary = stem_source_secondary[stem_name]

                stem_source_secondary = (
                    None if stem_value >= 4 else stem_source_secondary
                )
                self.write_to_console(
                    f"{SAVING_STEM[0]}{stem_name}{SAVING_STEM[1]}"
                ) if not self.is_secondary_model else None
                stem_path = os.path.join(
                    self.export_path, f"{self.audio_file_base}_({stem_name}).wav"
                )
                stem_source = spec_utils.normalize(
                    source[stem_value], self.is_normalization
                ).T
                self.write_audio(
                    stem_path,
                    stem_source,
            ...
            if not self.is_secondary_stem_only:
                ...
                primary_stem_path = os.path.join(
                    self.export_path,
                    f"{self.audio_file_base}_({self.primary_stem}).wav",
                )
                if not isinstance(self.primary_source, np.ndarray):
                    self.primary_source = spec_utils.normalize(
                        source[self.demucs_source_map[self.primary_stem]],
            ...
            if not self.is_primary_stem_only:
                ...
                def secondary_save(
                    sec_stem_name, source, raw_mixture=None, is_inst_mixture=False
                ):
                    secondary_source = (
                        self.secondary_source if not is_inst_mixture else None
                    )
                    self.write_to_console(
                        f"{SAVING_STEM[0]}{sec_stem_name}{SAVING_STEM[1]}"
                    ) if not self.is_secondary_model else None
                    secondary_stem_path = os.path.join(
                        self.export_path,
                        f"{self.audio_file_base}_({sec_stem_name}).wav",
                    )
                    ...
                    if not isinstance(secondary_source, np.ndarray):
                        if self.is_demucs_combine_stems:
                            ...
                            for i in source:
                                secondary_source += i
                            secondary_source = spec_utils.normalize(
                                secondary_source, self.is_normalization
                            ).T
                        else:
                            if not isinstance(raw_mixture, np.ndarray):
                                raw_mixture = prepare_mix(
                                    self.audio_file,
                                    self.chunks_demucs,
                                    self.margin_demucs,
                                    is_missing_mix=True,
                            ...
                            secondary_source, raw_mixture = (
                                spec_utils.normalize_two_stem(
                                    source[self.demucs_source_map[self.primary_stem]],
                                    raw_mixture,
                                    self.is_normalization,
                                )
                            ...
                            if self.is_invert_spec:
                                secondary_source = spec_utils.invert_stem(
                                    raw_mixture, secondary_source
                                )
                            else:
                                raw_mixture = spec_utils.reshape_sources(
                                    secondary_source, raw_mixture
                                )
                    ...
                    if not is_inst_mixture:
                        ...
                        self.secondary_source_map = {
                            self.secondary_stem: self.secondary_source
                        }
                ...
                if (
                    ...
                    secondary_save(
                        f"{self.secondary_stem} {INST_STEM}",
                        source,
                        raw_mixture=inst_raw_mix,
    ...
    def demix_demucs(self, mix):
        ...
        for nmix in mix:
            ...
            self.set_progress_bar(
                0.1, (0.8 / len(mix) * self.progress_value)
            ) if self.is_chunk_demucs else None
            cmix = mix[nmix]
    ...
    def seperate(self):
        if self.primary_model_name == self.model_basename and self.primary_sources:
            ...
            y_spec, v_spec = self.inference_vr(
                self.loading_mix(), device, self.aggressiveness
            )
            self.write_to_console(DONE, base_text="")
        ...
        if not self.is_secondary_stem_only:
            ...
            if not isinstance(self.primary_source, np.ndarray):
                self.primary_source = spec_utils.normalize(
                    self.spec_to_wav(y_spec), self.is_normalization
                ).T
        ...
        if not self.is_primary_stem_only:
            ...
            if not isinstance(self.secondary_source, np.ndarray):
                ...
                self.secondary_source = spec_utils.normalize(
                    self.spec_to_wav(v_spec), self.is_normalization
                ).T
                if not self.model_samplerate == 44100:
    ...
    def spec_to_wav(self, spec):
        ...
        if self.high_end_process.startswith("mirroring"):
            ...
            wav = spec_utils.cmb_spectrogram_to_wave(
                spec, self.mp, self.input_high_end_h, input_high_end_
            )
        else:
...
def process_secondary_model(
    ...
    if (
        type(secondary_sources) is dict
        and not is_4_stem_demucs
        and not is_pre_proc_model
    ):
        return gather_sources(
            secondary_model.primary_model_primary_stem,
            STEM_PAIR_MAPPER[secondary_model.primary_model_primary_stem],
            secondary_sources,
        )
    else:
        return secondary_sources
