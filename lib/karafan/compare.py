...
def calculate(reference, estimate):
    ...
    if estimates.shape != references.shape:
        print(
            "Warning: Different audio lengths : {} != {} !".format(
...
def SDR(song_output_path, output_format, Gdrive, elapsed_time):
    ...
    instrum = os.path.join(
        MultiSong, song_name[-3:] + "_instrum.flac"
    )  # Get only the number of the song
    vocals = os.path.join(MultiSong, song_name[-3:] + "_vocals.flac")

    if not os.path.exists(MultiSong):
        print("No Multi-Song folder found !")
        return
    if not os.path.exists(instrum) or not os.path.exists(vocals):
        print(f'No References Stems for "{song_name}" found !')
        return
    ...
    if len(Extracted_files) == 0:
        print("Check output folder. Cant find any files !")
        return
    ...
    Results = ""
    References["instrum"], _ = sf.read(
        os.path.join(MultiSong, song_name[-3:] + "_instrum.flac"), dtype="float32"
    )  # Get only the number of the song
    References["vocals"], _ = sf.read(
        os.path.join(MultiSong, song_name[-3:] + "_vocals.flac"), dtype="float32"
    )
    ...
    for extract in Extracted_files:
        ...
        if "Bleedings" in file_name:
            continue  # Skip Bleedings
        ...
        if song_score is None:
            ...
            print(
                ...
                + file_name
                + ("&nbsp;" * pad)
                + "SDR : <b>{:9.6f}</b>".format(song_score)
            )
            Results += (
                file_name + (" " * pad) + "SDR : {:9.6f}".format(song_score) + "\n"
            )
    ...
    if Results != "":
        with open(
            os.path.join(song_output_path, "SDR_Results.txt"), "a", encoding="utf-8"
        ) as file:
            file.write(
