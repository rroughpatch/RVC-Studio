...
def Get(font_size):
    ...
    match system:
            ...
            try:
                cpu_info = subprocess.check_output(
                    [
                        ...
                        "cpu",
                        "get",
                        "Caption,MaxClockSpeed,NumberOfCores,NumberOfLogicalProcessors",
                        "/FORMAT:CSV",
                    ],
                    shell=True,
        ...
        case "Linux":  # use 'lscpu'
            try:
                cpu_info = subprocess.check_output(
                    ["lscpu", "-J"], shell=True, stderr=subprocess.STDOUT
                ).decode("utf-8")
                try:
                            ...
                            if "field" in item and "data" in item:
                                ...
                                match item["field"]:
                                    ...
                                    case "Model name:":
                                        html += f"CPU : {data}<br>"
                                    case "CPU max MHz:":
                                        html += f"MaxClock Speed : {int(data)} MHz<br>"
                                    case "Socket(s):":
                                        sockets = int(data)
        ...
        case "Darwin":  # For macOS, use 'sysctl'
            try:
                ...
                html += "CPU : " + subprocess.check_output(
                    ["sysctl", "machdep.cpu"], shell=True, stderr=subprocess.STDOUT
                ).decode("utf-8")
            except FileNotFoundError:
    ...
    try:
        ...
        gpu_info = subprocess.check_output(
            nvidia_smi, shell=True, stderr=subprocess.STDOUT
        ).decode("utf-8")
        ...
        print(
            f"Erreur lors de l'exécution de nvidia-smi : {e.returncode}\n{e.output.decode('utf-8')}"
        )
    except FileNotFoundError:
