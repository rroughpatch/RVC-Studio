...
def Load_Audio(file, sample_rate, ffmpeg=None, output_path=None):
    ...
    # Corrupted file : try to correct it with ffmpeg
        ...
        print(
            '<div style="font-size:18px;color:#ff0040;"><b>Your audio file is Bad encoded ! Trying to correct ...</b></div>'
        )
        try:
            ...
            if not os.path.exists(corrected):
                subprocess.run(
                    f'"{ffmpeg}" -y -i "{file}" -codec:a flac -compression_level 5 -ch_mode mid_side -lpc_type cholesky -lpc_passes 1 -exact_rice_parameters 1 "{corrected}"',
                    shell=True,
                    text=True,
                    capture_output=True,
                    check=True,
...
def Save_Audio(file_path, audio, sample_rate, output_format, cut_off, ffmpeg):
    ...
    if output_format == "WAV":
        sf.write(
            ...
            sample_rate,
            format="wav",
            subtype="PCM_16" if "int" in audio.dtype.name else "FLOAT",
        )
    else:
        ...
        try:
            subprocess.run(
                ffmpeg, shell=True, text=True, capture_output=True, check=True
            )
    ...
    for i in range(0, audio_length, window_frame):
        ...
        RMS = np.max(
            librosa.amplitude_to_db(
                librosa.feature.rms(
                    y=audio[:, i : (i + window_frame)],
                    frame_length=window_frame,
                    hop_length=window_frame,
                )
            )
        )
        if RMS < threshold_dB:
...
def Linkwitz_Riley_filter(type, cutoff, audio, sample_rate, order=8):
    ...
    sos = signal.butter(
        order // 2, normal_cutoff, btype=type, analog=False, output="sos"
    )
...
def Change_sample_rate(audio, way, current_cutoff, target_cutoff):
    ...
    pitched_audio = librosa.resample(
        audio,
        orig_sr=current_cutoff * 2,
        target_sr=target_cutoff * 2,
        res_type="kaiser_best",
        axis=1,
    )
