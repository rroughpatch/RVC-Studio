...
def demix_mdxv3(mix, model, device, config, overlap_MDX23):
    ...
    mix = torch.cat(
        [
            torch.zeros(2, C - H, dtype=torch.float32),
            mix,
            torch.zeros(2, pad_size + C - H, dtype=torch.float32),
        ],
        1,
    )
    mix = mix.to(device)
    ...
    X = (
        torch.zeros(S, *mix.shape, dtype=torch.float32).to(device)
        if S > 1
        else torch.zeros_like(mix, dtype=torch.float32)
    )

    with torch.cuda.amp.autocast(
        dtype=torch.float32
    ):  # BUG fix : float16 by default !!
        with torch.no_grad():
            cnt = 0
    ...
    if S > 1:
        return {
            k: v
            for k, v in zip(
                config.training.instruments, estimated_sources.cpu().numpy()
            )
        }
    else:
...
def demix_base(mix, device, model, infer_session):
    ...
    mix_p = np.concatenate(
        (np.zeros((2, trim)), mix, np.zeros((2, pad)), np.zeros((2, trim))), 1
    )
...
class MusicSeparationModel:
    def __init__(self, config, cache_dir=None, use_cache=True, format: str = None):
        ...
        self.output_format = (
            config["AUDIO"]["output_format"] if format is None else format.upper()
        )
        # Integers
        ...
        self.low_pass = 14000 + (
            int(config["PROCESS"]["low_pass"]) * 500
        )  # from 14 Khz to 22 Khz
        ...
        self.output = (
            os.path.join(BASE_CACHE_DIR, "temp", "karafan")
            if cache_dir is None
            else cache_dir
        )

        self.ffmpeg = os.path.join(self.Gdrive, "ffmpeg") + (
        ...
        with open(os.path.join(CWD, "Data", "Models.csv")) as csvfile:
            ...
            for row in reader:
                ...
                if (
                    name == config["PROCESS"]["music_1"]
                    or name == config["PROCESS"]["music_2"]
                ):
                    row["Compensation"] = float(row["Vol_Comp"])
                    self.models["music"].append(row)

                if (
                    name == config["PROCESS"]["vocal_1"]
                    or name == config["PROCESS"]["vocal_2"]
                ):
                    row["Compensation"] = float(row["Vol_Comp"])
                ...
                if (
                    name == config["PROCESS"]["bleed_1"]
                    or name == config["PROCESS"]["bleed_2"]
                ):
                    row["Compensation"] = float(row["Vol_Comp"])
                ...
                if (
                    name == config["PROCESS"]["bleed_3"]
                    or name == config["PROCESS"]["bleed_4"]
                ):
                    row["Compensation"] = float(row["Vol_Comp"])
                    self.models["bleed_vocal"].append(row)

                if (
                    name == config["PROCESS"]["bleed_5"]
                    or name == config["PROCESS"]["bleed_6"]
                ):
                    row["Compensation"] = float(row["Vol_Comp"])
    ...
    def SEPARATE(self, audio_file, BATCH_MODE=False):
        ...
        original_audio, self.sample_rate = audio_utils.Load_Audio(
            audio_file, 44100, self.ffmpeg, self.song_output_path
        )

        if original_audio is None:
            print(
                "► Error : Can't correct your audio file !"
            )  # Too much Corrupted file !
            return
        else:
            print(f"Audio loaded: {original_audio.shape=} {self.sample_rate=}")
        ...
        print(
            f"{original_audio.shape[-1] // 44100} sec. - Rate : {self.sample_rate} Hz / Cut-OFF : {self.original_cutoff} Hz"
        )
        ...
        if len(self.models["music"]) == 0:
            ...
            if len(music_extracts) == 1:
                ...
                music_ensemble = audio_utils.Make_Ensemble(
                    "Max", music_extracts
                )  # Algorithm ?
        ...
        vocal_extracts = []
        vocal_sub = (
            normalized if music_ensemble is None else normalized - music_ensemble
        )
        ...
        if len(vocal_extracts) == 1:
            ...
            vocal_ensemble = audio_utils.Make_Ensemble(
                "Max", vocal_extracts
            )  # MAX and not average, because it's Vocals !!
        ...
        if len(self.models["bleed_music"]) == 0:
            ...
            for model in self.models["bleed_music"]:
                ...
                if audio is None:
                    audio = self.Extract_with_Model(
                        "Bleed_Music", vocal_ensemble, model
                    )
        ...
        print(
            f"► Apply High {self.high_pass} Hz & Low {self.low_pass / 1000:.1f} KHz Pass on Vocals"
        )

        if self.high_pass > 0:
            vocal_final = audio_utils.Pass_filter(
                "highpass", self.high_pass, vocal_final, self.sample_rate, order=16
            )

        if self.low_pass < 22000:
            if self.low_pass > 17000:
                vocal_final = audio_utils.Pass_filter(
                    "lowpass", self.low_pass, vocal_final, self.sample_rate, order=16
                )
            else:
                vocal_final = audio_utils.Pass_filter(
                    "lowpass", self.low_pass, vocal_final, self.sample_rate, order=8
                )
        ...
        if len(self.models["bleed_vocal"]) == 0:
            ...
            if self.DEBUG:
                self.Save_Audio("4 - Music - SUB", music_sub)
            ...
            if len(self.models["remove_music"]) > 0:
                for model in self.models["remove_music"]:
                    ...
                    if audio is None:
                        audio = self.Extract_with_Model(
                            "Bleed_Music", bleed_ensemble, model
                        )
                ...
                if len(music_extracts) == 1:
                    ...
                    bleed_ensemble = bleed_ensemble - audio_utils.Make_Ensemble(
                        "Max", music_extracts
                    )  # Algorithm ?
        ...
        if self.infra_bass:
            vocal_final = audio_utils.Pass_filter(
                "highpass", 18, vocal_final, self.sample_rate, order=100
            )

        # Apply silence filter
        if self.silent < 0:
            vocal_final = audio_utils.Silent(vocal_final, self.sample_rate, self.silent)
        ...
        # Apply Infra Bass filter
        if self.infra_bass:
            music_final = audio_utils.Pass_filter(
                "highpass", 18, music_final, self.sample_rate, order=100
            )

        # Apply silence filter
        if self.silent < 0:
            music_final = audio_utils.Silent(music_final, self.sample_rate, self.silent)
        ...
        if self.SDR_Testing:
            ...
            print("----------------------------------------")
            compare.SDR(
                self.song_output_path, self.output_format, self.Gdrive, elapsed_time
            )
    ...
    def Extract_with_Model(self, type, audio, model):
        ...
        if type == "Music":
            quality = self.Quality_Music
            text = "Extract Music"
        elif type == "Vocal":
            quality = self.Quality_Vocal
            text = "Extract Vocals"
        elif type == "Bleed_Music":
            quality = self.Quality_Bleed
            text = "Remove Music Bleedings"
        elif type == "Bleed_Vocal":
            quality = self.Quality_Bleed
            text = "Vocal Bleedings in Music"
        elif type == "Remove_Music":
            quality = self.Quality_Bleed
            text = "Music in Vocal Bleedings"
        ...
        if model["Stem"] == "BOTH":
            ...
            overlap = (
                self.MDX23_overlap
                if (type == "Music" or type == "Vocal")
                else self.MDX23_bleed
            )
            ...
            if self.Denoise:
                ...
                source = (
                    0.5
                    * -self.demix_full(-audio, mdx_model, inference, bigshifts, 1)[0]
                )
                source += (
                    0.5 * self.demix_full(audio, mdx_model, inference, bigshifts, 2)[0]
                )
            else:
            ...
            if quality["BigShifts_SRS"] > 0:
                ...
                if (
                    model["Cut_OFF"] > 0 and model["Name"] != "Vocal Main"
                ):  # Exception !!
                    ...
                    audio_SRS = audio_utils.Change_sample_rate(
                        audio, "DOWN", self.original_cutoff, model["Cut_OFF"] + delta
                    )

                    # Limit audio to the same frequency cut-off than MDX model : To avoid SRS noise !! (That helps a little bit)
                    audio_SRS = audio_utils.Pass_filter(
                        "lowpass",
                        model["Cut_OFF"],
                        audio_SRS,
                        self.sample_rate,
                    ...
                    if self.Denoise:
                        ...
                        source_SRS = 0.5 * audio_utils.Change_sample_rate(
                            -self.demix_full(
                                -audio_SRS, mdx_model, inference, bigshifts, 1
                            )[0],
                            "UP",
                        ...
                        source_SRS += 0.5 * audio_utils.Change_sample_rate(
                            self.demix_full(
                                audio_SRS, mdx_model, inference, bigshifts, 2
                            )[0],
                            "UP",
                            self.original_cutoff,
                            model["Cut_OFF"] + delta,
                        ...
                        # ONLY 1 Pass, for testing purposes
                        print(
                            f"{text} -> SRS High ({bigshifts} pass) - <b>NO Denoise !</b>"
                        )
                        source_SRS = audio_utils.Change_sample_rate(
                            self.demix_full(
                                audio_SRS, mdx_model, inference, bigshifts, 0
                            )[0],
                            "UP",
                            self.original_cutoff,
                            model["Cut_OFF"] + delta,
                    ...
                    source_SRS = librosa.util.fix_length(
                        source_SRS, size=source.shape[-1]
                    )
                    ...
                    if type == "Vocal":
                        ...
                        source = audio_utils.Linkwitz_Riley_filter(
                            "lowpass", 16000, source, self.sample_rate, order=12
                        ) + audio_utils.Linkwitz_Riley_filter(
                            "highpass", 16000, source_SRS, self.sample_rate, order=12
                        )
                ...
                if type == "Vocal":
                    ...
                    audio_SRS = audio_utils.Change_sample_rate(
                        audio, "UP", self.original_cutoff, cut_freq
                    )
                    ...
                    if self.Denoise:
                        ...
                        source_SRS = 0.5 * audio_utils.Change_sample_rate(
                            -self.demix_full(-audio_SRS, mdx_model, inference, 1, 1)[0],
                            "DOWN",
                            self.original_cutoff,
                            cut_freq,
                        )
                        ...
                        source_SRS += 0.5 * audio_utils.Change_sample_rate(
                            self.demix_full(audio_SRS, mdx_model, inference, 1, 2)[0],
                            "DOWN",
                            self.original_cutoff,
                            cut_freq,
                        )
                        ...
                        source_SRS = audio_utils.Change_sample_rate(
                            self.demix_full(audio_SRS, mdx_model, inference, 1, 0)[0],
                            "DOWN",
                            self.original_cutoff,
                            cut_freq,
                        )
    ...
    def Load_MDX(self, model):
        ...
        if name not in self.MDX:
            self.MDX[name] = {}
            self.MDX[name]["model"] = tfc_tdf.Conv_TDF_net_trim_model(
                self.GPU_device, "", 11, model
            )  # can use '*' to get 4 stems ??
    ...
    def Check_Already_Processed(self, key, model_name=""):
        ...
        """
        if not self.GOD_MODE or key not in self.AudioFiles_Debug:
            return None
    ...
    def Save_Audio(self, key, audio, model_name=""):
        ...
        if type(key) is int:
            if self.DEBUG:
                if (
                    key not in self.AudioFiles_Debug
                    and key not in self.AudioFiles_Mandatory
                ):
                    return
            else:
                if key not in self.AudioFiles_Mandatory:
                    return
    ...
    def demix_full(self, mix, use_model, infer_session, bigshifts, pass_number=0):
        ...
        if bigshifts < 1:
            bigshifts = 1  # must not be <= 0 !
        if bigshifts > mix_length:
            bigshifts = mix_length - 1
        ...
        while bigshifts * demix_seconds > mix_length:
            demix_seconds -= 1
        ...
        for shift in shifts:
            ...
            shifted_mix = np.concatenate(
                (mix[:, -shift_samples:], mix[:, :-shift_samples]), axis=-1
            )
            ...
            for i in range(0, shifted_mix.shape[-1], self.chunk_size):
                ...
                sources = demix_base(
                    mix_part, self.GPU_device, use_model, infer_session
                )
            ...
            result = np.concatenate(
                (result[..., shift_samples:], result[..., :shift_samples]), axis=-1
            )
...
def Process(
    input_file, cache_dir=None, use_cache=True, format=None, config=settings.Defaults
):
    model = MusicSeparationModel(
        config, cache_dir=cache_dir, use_cache=use_cache, format=format
    )
