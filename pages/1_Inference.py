...
from webui.components import (
    file_uploader_form,
    initial_vocal_separation_params,
    initial_voice_conversion_params,
    save_vocal_separation_params,
    save_voice_conversion_params,
    vocal_separation_form,
...
def one_click_convert(state):
    ...
    if state.input_vocals is not None and state.input_instrumental is not None:
        ...
        changed_vocals = call_rvc(state.model_name, state.input_vocals, **(params))

        if changed_vocals:
            state.output_vocals = changed_vocals
            mixed_audio = merge_audio(
                changed_vocals, state.input_instrumental, sr=state.input_instrumental[1]
            )
...
def download_song(output_audio, output_audio_name, ext="mp3", to_stereo=False):
    ...
    output_file = os.path.join(
        output_dir, ".".join([audio_path[1], "stereo" if to_stereo else "mono", ext])
    )
...
def render_vocal_separation_form(state):
    with st.form("inference.split_vocals.expander"):
        ...
        elif state.uvr5_params.uvr_models is None:
            st.write(i18n("inference.uvr_models"))
    return state
...
if __name__ == "__main__":
    with SessionStateContext(
        "inference", initial_state=init_inference_state()
    ) as state:
        file_uploader_form(
            SONG_DIR,
            "Upload your songs",
        ...
        with st.container():
            ...
            if col1.button(
                i18n("inference.refresh_data.button"), use_container_width=True
            ):
                state = refresh_data(state)
                st.experimental_rerun()

            if col2.button(
                i18n("inference.one_click.button"),
                type="primary",
                use_container_width=True,
                disabled=not (
                    state.uvr5_params.uvr_models
                    and state.input_audio_name
        ...
        with st.expander(
            i18n("inference.split_vocals.expander"),
            expanded=not (state.input_audio_name and len(state.uvr5_params.uvr_models)),
        ):
            state = render_vocal_separation_form(state)

        if st.button(
            i18n("inference.split_vocals"),
            disabled=not (state.input_audio_name and len(state.uvr5_params.uvr_models)),
        ):
            state.input_vocals, state.input_instrumental, state.input_audio = call_uvr(
                audio_path=state.input_audio_name,
                device=state.device,
        ...
        with st.container():
            ...
            if state.input_instrumental is not None:
                ...
                col2.audio(
                    state.input_instrumental[0], sample_rate=state.input_instrumental[1]
                )

        st.subheader(i18n("inference.convert_vocals"))
        with st.expander(
            f"{i18n('inference.convert_vocals.expander')} voice={state.model_name}"
        ):
            state = render_voice_conversion_form(state)

        col1, col2 = st.columns(2)
        uploaded_vocals = col1.file_uploader(
            "Upload your own voice file (if you didn't use voice extraction)",
            type=SUPPORTED_AUDIO,
        )
        if uploaded_vocals is not None:
            input_audio = bytes_to_audio(uploaded_vocals.getvalue())
            state.input_vocals = remix_audio(input_audio, norm=True, to_int16=True)
            state.input_audio_name = uploaded_vocals.name
            del uploaded_vocals
        uploaded_instrumentals = col2.file_uploader(
            "Upload your own instrumental file (if you didn't use voice extraction)",
            type=SUPPORTED_AUDIO,
        )
        if uploaded_instrumentals is not None:
            input_audio = bytes_to_audio(uploaded_instrumentals.getvalue())
            state.input_instrumental = remix_audio(
                input_audio, norm=True, to_int16=True
            )
            state.input_audio_name = uploaded_instrumentals.name
            del uploaded_instrumentals

        if st.button(
            i18n("inference.convert_vocals"),
            disabled=not (state.input_vocals and state.model_name),
        ):
            with st.spinner(i18n("inference.convert_vocals")):
                params = dict(state.convert_params)
                params.update(resample_sr=int(state.input_instrumental[1]))

                output_vocals = call_rvc(state.model_name, state.input_vocals, **params)
            ...
            if state.output_audio is not None:
                ...
                if c2.button(i18n("inference.download.button")):
                    st.toast(
                        download_song(
                            state.output_audio,
                            state.output_audio_name,
                            ext=state.format,
                            to_stereo=state.channels == "stereo",
                        )
                    )
