...
def initial_state():
    return ObjectNamespace(remote_bind=False, host="localhost", port=5555)
    ...
    with SessionStateContext("rvc-api", initial_state()) as state:
        ...
        with st.form("rvc-api-form"):
            ...
            state.url = st.text_input(
                "Server URL", value=f"http://{state.host}:{state.port}"
            )

            if st.form_submit_button("Start Server", disabled=is_active):
                with ProgressBarContext(
                    [1], sleep, "Waiting for rvc api to load"
                ) as pb:
