...
def set_volume(state):
    if state.player and state.volume != state.player.volume:
        state.player.set_volume(state.volume)


def set_loop(state):
    if state.player and state.loop != state.player.loop:
        state.player.set_loop(state.loop)


def set_shuffle(state):
    if state.player:
        if state.shuffle:
            if not state.player.shuffled:
...
if __name__ == "__main__":
    with SessionStateContext("playlist", initial_state=init_inference_state()) as state:
        ...
        with st.expander(
            "Settings",
            expanded=not (state.player and len(state.split_vocal_config.uvr_models)),
        ):
        ...
        if state.player:
            ...
            df = pd.DataFrame({"Songs": state.player.playlist})
            index = np.arange(len(df))
            df.index = np.where(
                [
                    False
                    if state.player.current_song is None
                    else state.player.current_song in song
                    for song in state.player.playlist
                ],
                "⭐",
                index,
            )
