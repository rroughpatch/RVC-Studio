...
if __name__ == "__main__":
    with SessionStateContext("tensorboard") as state:
        state.url = st.text_input(
            "Tensorboard URL", value=state.url if state.url else "http://localhost:6006"
        )
        if state.url:
        ...
        state.logdir = st.text_input(
            "Path to Logs", value=state.logdir if state.logdir else LOG_DIR
        )
        state.remote_bind = st.checkbox(
            "Bind to 0.0.0.0(Required for docker or remote connections)",
            value=state.remote_bind if state.remote_bind else False,
        )
        if st.button("Start Tensorboard", disabled=tensorboard_is_active):
            with ProgressBarContext(
                [1] * 5, time.sleep, "Waiting for tensorboard to load"
            ) as pb:
                start_tensorboard(
                    state.logdir, "localhost" if not state.remote_bind else "0.0.0.0"
                )
                pb.run()
