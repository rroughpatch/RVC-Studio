...
def one_click_speech(state):
    speaker_name = (
        os.path.basename(state.model_name).split(".")[0] if state.model_name else None
    )
    state.tts_audio = generate_speech(
        state.tts_text,
        speaker=speaker_name,
        method=state.tts_method,
        device=state.device,
    )
    state.converted_voice = call_rvc(
        state.model_name, state.tts_audio, **(state.tts_options)
    )
...
if __name__ == "__main__":
    with SessionStateContext("tts", initial_state=init_inference_state()) as state:
        ...
        with st.container():
            state.tts_text = st.text_area(
                "Speech", value=state.tts_text if state.tts_text else "", max_chars=600
            )
            ...
            if col1.button("Generate Speech", disabled=not state.tts_text):
                speaker_name = (
                    os.path.basename(state.model_name).split(".")[0]
                    if state.model_name
                    else None
                )
                state.tts_audio = generate_speech(
            ...
            if state.tts_audio:
                ...
                if col2.button("Convert Speech"):
                    state.converted_voice = convert_vocals(
                        state.model_name, state.tts_audio, **(state.tts_options)
                    )

                if state.converted_voice:
                    col2.audio(
                        state.converted_voice[0], sample_rate=state.converted_voice[1]
                    )
