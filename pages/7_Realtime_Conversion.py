...
def load_model(_state):
    if (
        _state.rvc_models is None
        or _state.rvc_models["model_name"] != _state.voice_model
    ):
        del _state.rvc_models
        _state.rvc_models = get_vc(
            _state.voice_model, config=config, device=_state.device
        )
        gc_collect()
...
if __name__ == "__main__":
    with SessionStateContext("realtime-rvc", initial_state=init_state()) as state:
        ...
        state.input_device_index = col2.selectbox(
            "Input Device",
            options=INPUT_DEVICES,
            format_func=lambda i: (
                f"{i}. " + state.p.get_device_info_by_index(i)["name"]
            ),
            index=get_index(INPUT_DEVICES, state.input_device_index),
        )

        state.output_device_index = col3.selectbox(
            "Output Device",
        ...
        with st.expander(
            f"Voice Model: {state.recorder}", expanded=state.recorder is None
        ):
            with st.form("realtime-voice"):
                ...
                if st.form_submit_button(
                    ...
                    if state.recorder is None:
                        ...
                        state.recorder.load_rvc_model(
                            state.voice_model, config, state.device
                        )
