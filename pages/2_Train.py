...
def create_filelist(exp_dir, if_f0, spk_id, version, sr):
    ...
    if if_f0:
        ...
        names = set([os.path.splitext(name)[0] for name in os.listdir(feature_dir)])
    opt = []
    missing_data = []
    for name in names:
    ...
    num_mute = max(
        2, len(opt) // 100
    )  # use 1% mute file or 2 copies (like original repo)
...
if __name__ == "__main__":
    with SessionStateContext("training", init_training_state()) as state:
        with st.container():
            ...
            state.version = col2.radio(
                i18n("training.version"),
                options=["v2"],
                horizontal=True,
                index=get_index(["v2"], state.version),
            )
            state.device = col3.radio(
                i18n("training.device"), options=["cuda", "cpu"], horizontal=True
            )
            ...
            file_uploader_form(
                DATASETS_DIR,
                "Upload a zipped folder of your dataset (make sure the files are in a folder)",
                types="zip",
            )
            state.trainset_dir = st.text_input(
                i18n("training.preprocess_data.trainset_dir"),
                placeholder="./datasets/name_of_zipped_folder",
            )

            state.period = st.slider(
            ...
            state.overlap = st.slider(
                "Length of sample overlap (seconds)",
                min_value=0.3,
                max_value=3.0,
                step=0.1,
                value=state.overlap,
            ...
            disabled = not (
                state.trainset_dir
                and state.exp_dir
                and os.path.exists(state.trainset_dir)
            )
        ...
        PRETRAINED_G = get_filenames(
            root="models",
            folder="pretrained_v2",
            name_filters=["f0" if state.if_f0 else "k", "G", state.sr],
            filter_func=all,
        )
        PRETRAINED_D = get_filenames(
            root="models",
            folder="pretrained_v2",
            name_filters=["f0" if state.if_f0 else "k", "D", state.sr],
            filter_func=all,
        )
        model_log_dir = f"{state.exp_dir}_{state.version}_{state.sr}"

        with st.form(
            i18n("training.extract_features.form")
        ):  # extract_features(exp_dir, n_threads, version, if_f0, f0method)
            ...
            state.f0method = col2.multiselect(
                i18n("training.f0method"),
                options=PITCH_EXTRACTION_OPTIONS,
                default=state.f0method,
            )
            disabled = not (
                state.exp_dir
                and os.path.exists(os.path.join(LOG_DIR, model_log_dir, "1_16k_wavs"))
            )
            if st.form_submit_button(
                i18n("training.extract_features.submit"), disabled=disabled
            ):
                st.toast(
                    extract_features(
                        state.exp_dir,
        ...
        with st.form(
            i18n("training.train_model.form")
            ...
            state.gpus = st.multiselect(
                i18n("training.gpus"),
                options=[str(i) for i in range(torch.cuda.device_count())],
                default=state.gpus,
            )
            col1, col2, col3 = st.columns(3)
            state.batch_size = col1.slider(
                i18n("training.batch_size"),
                min_value=1,
                max_value=100,
                step=1,
                value=state.batch_size,
            )
            state.total_epoch = col2.slider(
                i18n("training.total_epoch"),
                min_value=0,
                max_value=1000,
                step=10,
                value=state.total_epoch,
            )
            state.save_epoch = col3.slider(
                i18n("training.save_epoch"),
                min_value=0,
                max_value=100,
                step=1,
                value=state.save_epoch,
            )
            state.pretrained_G = st.selectbox(
                i18n("training.pretrained_G"), options=PRETRAINED_G
            )
            state.pretrained_D = st.selectbox(
                i18n("training.pretrained_D"), options=PRETRAINED_D
            )
            state.if_save_latest = st.checkbox(
                i18n("training.if_save_latest"), value=state.if_save_latest
            )
            state.if_cache_gpu = st.checkbox(
                i18n("training.if_cache_gpu"), value=state.if_cache_gpu
            )
            state.if_save_every_weights = st.checkbox(
                i18n("training.if_save_every_weights"),
                value=state.if_save_every_weights,
            )

            disabled = not (
                state.exp_dir
                and os.path.exists(os.path.join(LOG_DIR, model_log_dir, "3_feature768"))
            )
            if st.form_submit_button(
                i18n("training.train_model.submit"), disabled=disabled
            ):
                if not (state.pretrained_D and state.pretrained_G):
                    st.toast("Please download the pretrained models!")
                else:
                    with ProgressBarContext(
                        [1] * 100,
                        sleep,
                        "Waiting for training process to spawn (you should hear your GPU fans spinning). Uncheck GPU cache if you have a large dataset.",
                    ) as pb:
                        st.toast(
                            train_model(
                                state.exp_dir,
        ...
        disabled = not (
            ...
            and os.path.exists(
                os.path.join(
                    LOG_DIR,
                    model_log_dir,
                    "3_feature256" if state.version == "v1" else "3_feature768",
                )
            )
        )
        if (
            state.exp_dir
            and state.version
            and st.button(i18n("training.train_index.submit"), disabled=disabled)
        ):
