...
def convert_song(
    ...
    **kwargs,
    ...
    song_path = os.path.join(
        cache_dir, os.path.basename(audio_path).split(".")[0] + ".mp3"
    )
    ...
    input_vocals, input_instrumental, input_audio = split_vocals(
        audio_path=audio_path, **split_audio_params
    )
...
class PlaylistPlayer:
    def __init__(
        ...
        **args,
        ...
        self.thread1 = threading.Thread(
            target=asyncio.run, args=(self.play_song(),), name="play_song", daemon=True
        )
        self.thread1.start()
        self.thread2 = threading.Thread(
            target=asyncio.run,
            args=(self.process_song(),),
            name="process_song",
            daemon=True,
        )  # process for converting songs
        self.thread2.start()
    ...
    def __repr__(self):
        status = (
            "paused"
            if self.paused
            else f"{'looping' if self.loop else 'playing'}: {self.current_song}"
        )
            ...
            with self.lock:
                if item is None:
                    ...
                    format = (
                        ...
                        else pyaudio.paFloat32
                    )
                    dtype = "int16" if np.abs(audio).max() > 1 else "float32"
                    try:
                        self.stream = p.open(
                            format=format, channels=1, rate=sr, output=True
                        )
                        ...
                        for i in range(0, len(audio), self.CHUNKSIZE):
                            if not (self.paused or self.stopped):
                                data = (
                                    audio[i : i + self.CHUNKSIZE] * self.volume
                                ).astype(dtype)
            ...
            with self.lock:
                if not self.queue.full():
                    ...
                    try:
                        ...
                        input_audio = convert_song(
                            song, self.model_name, use_cache=self.use_cache, **self.args
                        )
