...
from webui.api import (
    get_uvr_models,
    get_uvr_postprocess_models,
    get_uvr_preprocess_models,
)
...
def file_uploader_form(
    save_dir,
    title="Upload your files",
    types=None,
    params_mapper=__default_mapper,
    **kwargs,
):
    with st.container():
        with st.form(save_dir + title, clear_on_submit=True):
            ...
            if st.form_submit_button("Save") and files is not None:
                file_list = [
                    params_mapper(params)
                    for params in save_file_generator(
                        save_dir, files if isinstance(files, list) else [files]
                    )
                ]
    ...
    with st.expander(i18n("process.pids")):
        for p in get_subprocesses():
            ...
            try:
                ...
                if col4.button(
                    i18n("process.kill_one_pid"), key=f"process.kill_one_pid.{p.pid}"
                ):
    ...
    if folder:
        try:
            config_file = os.path.join(
                BASE_DIR, "configs", folder, "vocal_separation_params.json"
            )
...
def vocal_separation_form(state):
    ...
    state.preprocess_models = st.multiselect(
        ...
        default=[
            name for name in state.preprocess_models if name in uvr5_preprocess_models
        ],
    )
    ...
    state.postprocess_models = st.multiselect(
        ...
        format_func=lambda item: os.path.basename(item),
        default=[
            name for name in state.postprocess_models if name in uvr5_postprocess_models
        ],
    )
    ...
    state.agg = col1.slider(
        i18n("inference.agg"), min_value=0, max_value=20, step=1, value=state.agg
    )
...
def initial_voice_conversion_params(folder=None):
    ...
    if folder:
        try:
            config_file = os.path.join(
                BASE_DIR, "configs", folder, "voice_conversion_params.json"
            )
            os.makedirs(os.path.dirname(config_file), exist_ok=True)
            if os.path.isfile(config_file):
                with open(config_file, "r") as f:
...
def voice_conversion_form(state, use_hybrid=True):
    state.f0_up_key = st.slider(
        i18n("inference.f0_up_key"),
        min_value=-12,
        max_value=12,
        value=state.f0_up_key,
        step=1,
    )
    if use_hybrid:
        state.f0_method = st.multiselect(
            i18n("inference.f0_method"),
            options=PITCH_EXTRACTION_OPTIONS,
            default=state.f0_method,
        )
    else:
        state.f0_method = st.selectbox(
            i18n("inference.f0_method"),
            options=PITCH_EXTRACTION_OPTIONS,
    ...
    state.f0_autotune = col2.checkbox(
        i18n("inference.f0_autotune"), value=state.f0_autotune
    )
    state.resample_sr = st.select_slider(
        i18n("inference.resample_sr"),
        options=[0, 16000, 24000, 22050, 40000, 44100, 48000],
        value=state.resample_sr,
    )
    state.index_rate = st.slider(
        i18n("inference.index_rate"),
        min_value=0.0,
        max_value=1.0,
        step=0.05,
        value=state.index_rate,
    )
    # state.filter_radius=st.slider(i18n("inference.filter_radius"),min_value=0,max_value=7,step=1,value=state.filter_radius)
    state.rms_mix_rate = st.slider(
        i18n("inference.rms_mix_rate"),
        min_value=0.0,
        max_value=1.0,
        step=0.05,
        value=state.rms_mix_rate,
    )
    state.protect = st.slider(
        i18n("inference.protect"),
        min_value=0.0,
        max_value=0.5,
        step=0.01,
        value=state.protect,
