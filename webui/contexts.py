...
class SessionStateContext:
    def __init__(self, name: str, initial_state={}):
        ...
        self.__initial_state__ = (
            ObjectNamespace() if initial_state is None else initial_state
        )

    def __enter__(self):
        if self.__name__ in st.session_state:
            return st.session_state[self.__name__]
        st.session_state[self.__name__] = self.__data__
        return self.__data__

    def __exit__(self, *_):
        if self.__name__ not in st.session_state:
            st.session_state[self.__name__] = self.__data__
        gc_collect()

    def __dir__(self):
        return dir(self.__data__)
...
class ProgressBarContext:
    ...
    def __runner__(self, index):
        ...
        self.progress += 1
        self.__progressbar__.progress(
            float(self.progress / self.max_progress),
            f"{self.text}: {self.progress}/{self.max_progress}",
        )
